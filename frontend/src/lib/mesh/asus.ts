/**
 * ASUS AiMesh provider — transforms raw ASUS API data into generic MeshTopology.
 * All data comes through the laubter-mesh rpcd proxy plugin.
 */

import { call } from '$api/ubus';
import type { MeshNode, MeshClient, MeshRadio, MeshTopology, MeshBand, MeshConfig, MeshProvider } from './types';
...
function bandFromKey(key: string): MeshBand {
	switch (key) {
		case '2G': return '2G';
		case '5G': return '5G';
		case '5G1': return '5G1';
...
// Build a map of MAC → { nodeMac, band, rssi } from allclientlist
function buildClientNodeMap(
	allClients: Record<string, Record<string, Record<string, AsusAllClientEntry>>>
): Map<string, { nodeMac: string; band: MeshBand; rssi: number }> {
	const map = new Map<string, { nodeMac: string; band: MeshBand; rssi: number }>();
	...
	for (const [nodeMac, bands] of Object.entries(allClients)) {
		for (const [bandKey, clients] of Object.entries(bands)) {
			if (bandKey === 'wired_mac') {
				// Wired clients under this node
				for (const [clientMac] of Object.entries(clients)) {
					map.set(clientMac.toLowerCase(), { nodeMac, band: '2G', rssi: 0 }); // band irrelevant for wired
				}
				continue;
			}
...
export const asusProvider: MeshProvider = {
	name: 'asus',
	...
	async getTopology(): Promise<MeshTopology> {
		...
		const nodes: MeshNode[] = nodesRaw.map((raw) => {
			...
			// Count clients per radio from topology data
			const nodeTopology = topologyRaw[raw.mac] || {};

			const addRadio = (band: MeshBand, radioMac: string, ssid: string) => {
				if (!radioMac) return;
			...
			return {
				...
				radios,
				// ASUS re_path: 0=main node, 1=wired backhaul, 2=wireless backhaul
				backhaulType: raw.re_path === '2' ? 'wireless' : raw.re_path === '1' ? 'wired' : 'wired',
				// Link rate from wired_port: Q=2.5G, G=1G, M=100M
				linkRate: (() => {
		...
		for (const [mac, raw] of Object.entries(clientsRaw)) {
			...
			// Skip mesh node MACs (they appear as clients too)
			...
			const parentName = nodeAliasMap.get(parentMac) || parentMac;
				...
				// ASUS isWL values: "0"=wired, "1"=2.4GHz, "2"=5GHz, "3"=5GHz-2, "4"=6GHz
